...
func player_hit() -> void:
	iframes = max_iframes
	t_iframe.start()
	info.char_current_anim_sprite.modulate = red_hit
...
func _on_iframe_cd_timeout() -> void:
	...
	tween.tween_property(info.char_current_anim_sprite, "modulate", gray_faded, 0.1)
	tween.tween_property(info.char_current_anim_sprite, "modulate", white, 0.1)
